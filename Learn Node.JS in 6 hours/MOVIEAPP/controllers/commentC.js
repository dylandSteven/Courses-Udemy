...
const postComment = (req, res, next) => {
  ...
  commentData.userId = new ObjectId(req.user["id"]);
  ...
  comment
    ...
    .then(() => {
      res.status(201).json({
        message: "the comment has been successfully creates",
      });
    })
...
const putComment = (req, res, next) => {
  ...
  const commentId = new Object(req.params.commentId);
...
const getComments = (req, res, next) => {
  ...
  if (isNaN(pageXOffset)) {
    return next(createError(400));
  }

  const commentsToSkip = pageNum * 10;
  dbCon("commments", async (db) => {
    try{
